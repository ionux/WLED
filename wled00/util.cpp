...
void sappend(char stype, const char* key, int val)
{
  ...
  switch(stype)
  {
    // RLM - check return values on all of the oappend function calls below
    case 'c': //checkbox
      oappend(ds);
      oappend(key);
      oappend(".checked=");
...
bool oappendi(int i)
{
  ...
  // RLM - verify return values on these functions
  sprintf(s, "%d", i);
...
{
  // RLM - validate input and verify return values on these functions
  ...
  if (pos > 5)
  {
    // RLM - verify range bounds for hostname[] array
...
uint8_t extractModeName(uint8_t mode, const char *src, char *dest, uint8_t maxLen)
{
  ...
  if ((src == JSON_mode_names) || (src == nullptr))
  {
    if (mode < strip.getModeCount())
    {
      ...
      {
        if ((lineBuffer[j] == '\0') || (lineBuffer[j] == '@'))
        {
          break;
        }
  ...
  for (size_t i = 0; i < len; i++)
  {
    ...
    if (singleJsonSymbol == '\0')
    {
      break;
    }

    if (singleJsonSymbol == '@' && insideQuotes && qComma == mode)
    {
      break; //stop when SR extension encountered
    }
    ...
    if ((qComma > mode) || (printedChars >= maxLen))
    {
      break;
    }
  }
      ...
      if ((start > 0) && (stop > 0))
      {
        ...
        else if (slider == 255)
        {
          ...
          if (nameBegin >= 0)
          {
            ...
            if (!isdigit(names[nameBegin+1]))
            {
              nameBegin = names.indexOf('=', nameBegin + 1); // look for default value
            }

            if ((nameEnd >= 0) && (nameBegin > nameEnd))
            {
              nameBegin = -1;
            }
        ...
        {
          // RLM - addressing safety code smell
        }
...
void checkSettingsPIN(const char* pin)
{
  ...
  if (!correctPIN && ((millis() - lastEditTime) < PIN_RETRY_COOLDOWN))
  {
    return; // guard against PIN brute force
  }
...
{
  static uint16_t volumeRaw = 0;

  static uint8_t samplePeak = 0;
  static uint8_t maxVol     = 0;
  static uint8_t binNum     = 0;

  static float FFT_MajorPeak = 0.0f;
  static float volumeSmth    = 0.0f;
  static float my_magnitude  = 0.0f;
