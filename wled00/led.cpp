...
void applyValuesToSelectedSegs()
{
  ...
  for (uint8_t i = 0; i < strip.getSegmentsNum(); i++)
  {
    ...
    if ((i != firstSel) && (!seg.isActive() || !seg.isSelected()))
    {
      continue;
    }
...
void applyBri()
{
  // RLM - refactor first condition to be boolean
...
void stateUpdated(byte callMode)
{
  ...
  if (bri != briOld || stateChanged)
  {
    ...
    if ((callMode != CALL_MODE_NOTIFICATION) && (callMode != CALL_MODE_NO_NOTIFY))
    {
      notify(callMode);
    }

    if ((bri != briOld) && nodeBroadcastEnabled)
    {
      sendSysInfoUDP(); // update on state
    }
  ...
  if (briT == 0)
  {
    ...
    {
      resetTimebase(); //effect start from beginning
    }
  }
  ...
  if ((bri == nightlightTargetBri) && (callMode != CALL_MODE_NO_NOTIFY) && (nightlightMode != NL_MODE_SUN))
  {
  ...
  if (fadeTransition)
  {
    ...
    if ((callMode != CALL_MODE_NOTIFICATION) && !jsonTransitionOnce)
    {
      transitionDelayTemp = transitionDelay; // load actual transition duration
    }
...
void handleNightlight()
{
  ...
  if ((now < 100) && (lastNlUpdate > 0))
  {
    lastNlUpdate = 0; // take care of millis() rollover
  }

  if ((now - lastNlUpdate) < 100)
  {
    return; // allow only 10 NL updates per second
  }
  ...
  if (nightlightActive)
  {
    ...
    {
      nightlightStartTime = millis();
      nightlightDelayMs = (uint32_t)(nightlightDelayMins * 60000);
      nightlightActiveOld = true;
      briNlT = bri;

      // RLM - unroll loop refactor
      for (byte i = 0; i < 4; i++)
      {
    ...
    // RLM - refactor div, also does this have to be a float?
    ...
    if (nightlightMode == NL_MODE_FADE || nightlightMode == NL_MODE_COLORFADE)
    {
      ...
      // color fading only is enabled with "NF=2"
      if (nightlightMode == NL_MODE_COLORFADE)
      {
        // RLM - unroll loop
        for (byte i = 0; i < 4; i++)
        {
          // fading from actual color to secondary color
          col[i] = colNlT[i] + ((colSec[i] - colNlT[i]) * nper);
        }
      }
