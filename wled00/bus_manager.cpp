...
uint8_t *Bus::allocData(size_t size)
{
  ...
  // RLM - can return NULL, see manpage.
  return _data = (uint8_t *)((size > 0)
      ...
      _colorOrderMap(com)
{
  ...
  if (IS_2PIN(bc.type))
  {
    ...
    _frequencykHz = (bc.frequency)
                      ? bc.frequency
                      : 2000U; // 2MHz clock if undefined
  }
  ...
  if (bc.doubleBuffer && !allocData(bc.count * (Bus::hasWhite(_type) + (3 * Bus::hasRGB(_type)))))
  {
    return; // Warning: hardcoded channel count
  }
...
// Fix for turning off onboard LED breaking bus
#ifdef LED_BUILTIN
  // && b > 0, covered by guard if above
  if (_bri == 0)
  {
    if (_pins[0] == LED_BUILTIN || _pins[1] == LED_BUILTIN)
    {
      reinit();
    }
  }
  ...
  if (_type == TYPE_WS2812_1CH_X3)
  {
    hwLen = NUM_ICS_WS2812_1CH_3X(_len); // only needs a third of "RGB" LEDs for NeoPixelBus
  }

  // RLM - Glad to see this typedef. Wanted to investigate their usage everywhere anyway.
  for (uint_fast16_t i = 0; i < hwLen; i++)
  {
    // RLM - Can we eliminate this temp var and nest the function call?
...
void IRAM_ATTR BusDigital::setPixelColor(uint16_t pix, uint32_t c)
{
  ...
  if (_cct >= 1900)
  {
    // Color correction from CCT
    c = colorBalanceFromKelvin(_cct, c);
  }

  // should be _data != nullptr, but that causes ~20% FPS drop
  // RLM - same as above, investigate why
  if (_buffering)
  {
    // RLM - I'm seeing this '(3 * Bus::hasRGB(_type)' code snippet a lot. Maybe can be precomputed and optimized?
    size_t channels = Bus::hasWhite(_type) + (3 * Bus::hasRGB(_type));
  ...
  if (_type == TYPE_WS2812_1CH_X3)
  {
    ...
    switch (pix % 3)
    {
      ...
      case 2:
        ...
        break;
      default:
        // RLM - added for code safety & code smell
        break;
    }
  }
...
// RLM - validate all memory deallocation here!!!
...
void BusPwm::setPixelColor(uint16_t pix, uint32_t c)
{
  ...
  {
    // RLM - investigate refactoring div
    ww = ((255 - cct) * 255) / (255 - _cctBlend);
  }

  if ((255 - cct) < _cctBlend)
  {
    cw = 255;
  }
  else
  {
    cw = (cct * 255) / (255 - _cctBlend);
  }

  // RLM - investigate refactoring these divs
  ww = (w * ww) / 255; //brightness scaling
  cw = (w * cw) / 255;
#endif

  // RLM - some cases bellow fall through but aren't documented.  ensure not a bug
  switch (_type)
  {
    ...
    case TYPE_ANALOG_3CH: //standard dumb RGB
      ...
      break;
    default:
      // RLM - added for safety code smell
      // RLM - maybe needs a debug here?
      break;
  }
}
...
//does no index check
// RLM - range check array bounds below!!!!
      ...
      _broadcastLock(false)
{
  ...
  // RLM - range check array bounds here
  _client = IPAddress(bc.pins[0], bc.pins[1], bc.pins[2], bc.pins[3]);

  // RLM - validate memory allocation!!!!
...
// RLM - validate it's not happening
...
uint16_t BusManager::getTotalLength()
{
  ...
  for (uint8_t i = 0; i < numBusses; i++)
  {
    // RLM - validate array bounds and validate b is not NULL!!
    len += busses[i]->getLength();
  }
...
// RLM - investigate if we can refactor these static members
int16_t Bus::_cct = -1;
